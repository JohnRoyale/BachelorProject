...
public class Main {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		long equal=0;
		int j = 1;
		for (int i = controller.getEpoch(); i < 200; i++) {
			int k = 0;
			gameTime = System.currentTimeMillis();
			draw = i % 10 == 0;

			while (!controller.gameOver() && k < 90 * 50) {
				if ((System.currentTimeMillis() - ctime > 20) || (controller.queueFull())) {
			...
			System.out.println("Neural AI won " + wins + " games drawed "+equal+" games out of the last " + j + " games played\n");
